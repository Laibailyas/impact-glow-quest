...
export function SecurityControl() {
  ...
  return (
    <section id="security" className="relative overflow-hidden bg-ink px-6 py-24 text-paper md:py-32">
      <motion.div
        aria-hidden
      ...
      <div className="relative mx-auto grid max-w-[1800px] grid-cols-1 items-start gap-14 lg:grid-cols-[0.9fr_1.1fr] lg:gap-20">
        <motion.div
        ...
        >
          ...
          <h2 className="mt-6 font-display text-[clamp(2.6rem,5.2vw,5rem)] leading-[0.92]">
            You stay
            <br />
          ...
          <div className="mt-10 flex items-center gap-5 rounded-2xl border border-paper/15 bg-paper/[0.04] p-5">
            <button
              type="button"
              data-cursor-hover
              onClick={() => setOn((v) => !v)}
          ...
          {CARDS.map((card, i) => {
            ...
            return (
              <motion.article
                ...
                whileHover={{ y: -6 }}
                className="group relative overflow-hidden rounded-3xl border border-paper/12 bg-paper/[0.04] p-7"
              >
                ...
                <h3 className="relative mt-6 font-display text-2xl uppercase leading-tight">{card.title}</h3>
